...
CPdirection <- function(npcs,X,x0,A,b,h){
  ...
  # input
  # npcs: A scalar defining the number of principal components requested
  # X: A matrix  representing data (each row corresponds to a variable, and column corresponds to a case)
  # x0: A vector defining the reference element
  # A: A matrix defining the constraint matrix
  # b: A vector defining the constraint vector
  # h: A scalar defining the central difference step size
...
ConvexPCA <- function(M, X, A, b){
  ...
  # Input 
  # M: A scalar defining the number of principal components requested
  # X: A matrix representing data (each row corresponds to a case, and column corresponds to a row)
  # A: A matrix defining the constraint matrix
  # b: A vector defining the constraint vector
  
  # Output
  # mean: A vector representing the mean of data 
  # directions: A matrix containing the convex principal directions 
  # scores: A matrix containing the convex principal component scores of data
  # variances: A vector containing the variances explained by the convex principal directions
  # finiteReps: A matrix containing the finite-dimensional representations of data
